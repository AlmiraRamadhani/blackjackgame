...
//                        Kartu k = main.listKartu.get(main.idxCard);
//                        main.nextCard();
                        Kartu k = main.listKartu.get(main.idxCard);
                        main.deadlcard();
                        String kartu = k.getJenis() + "~" + k.getNama() + "~" + k.getValue() + ";;";
                        DataOutputStream dos3 = new DataOutputStream(main.getClientList(username).getOutputStream());
                        dos3.writeUTF("GET_CARD " + kartu);
                        break;
                    ...
                    case "CMD_CHAT":
                        ...
                        try {
                            ...
                            main.appendMessage("[IOException]: tidak bisa mengirim pesan ke " + sendTo);
                        }
                        break;
                    ...
                    case "CMD_CLOSECARD":
                        ...
                        for (int x = 0; x < main.clientList.size(); x++) {
                            if (!main.clientList.elementAt(x).equals(player)) {
                                try {
                                    Socket tsoc2 = (Socket) main.socketList.elementAt(x);
                                    DataOutputStream dos2 = new DataOutputStream(tsoc2.getOutputStream());
                                    dos2.writeUTF("CMD_MESSAGE CARD CLOSE "+message);
                                } catch (IOException e) {
                                    main.appendMessage("[CMD_CHATALL]: " + e.getMessage());
                                }
                            }
                        }
                        main.appendMessage("[CMD_CHATALL]: " + player +" close card and get "+score);
                        break;
